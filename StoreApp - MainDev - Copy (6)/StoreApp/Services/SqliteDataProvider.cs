                ...
                using (SQLiteCommand cmd = new SQLiteCommand(sqlInsert, conn))
                {
                    using (SQLiteDataAdapter da = new SQLiteDataAdapter(cmd))
                    {
                        ...
                        foreach (DataRow row in dataTable.Rows)
                        {
                            ...
                            Item.Unit_Id = row["unit_id"] != null ? row["Cat_Id"].ToString() : string.Empty;
                            Item.Price = row["Price"] != null ?((float)row["Price"]) : 0;
                            Item.Start_Date = row["start_dt"] != null ? row["Cat_Id"].ToString() : string.Empty; ;
                            Item.End_Date = row["end_date"] != null ? row["Cat_Id"].ToString() : string.Empty; ;
                            Item.Active = row["active"] != null ? row["Cat_Id"].ToString() : string.Empty; ;
            ...
            using (SQLiteConnection conn = new SQLiteConnection(ConnectionString))
            {
                ...
                using (SQLiteCommand cmd = new SQLiteCommand(sqlInsert, conn))
                {
                    ...
                    if (row.Read())
                    {
                        ...
                        Item.End_Date = row["end_date"] != null ? row["end_date"].ToString() : string.Empty; ;
